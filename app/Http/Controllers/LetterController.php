...
class LetterController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'variables.*.type' => 'required|string|in:text,date,daterange,number,boolean,dropdown,richtext,calculated',
            'variables.*.dummy' => 'nullable|string',
            'variables.*.options' => 'nullable|string',
            'variables.*.formulas' => 'nullable|array',
            'variables.*.formulas.*.key' => 'required_with:variables.*.formulas|string',
            'variables.*.formulas.*.label' => 'nullable|string',
            'variables.*.formulas.*.expression' => 'required_with:variables.*.formulas|string',
            'margin_top' => 'required|integer|min:0|max:100',
            'margin_bottom' => 'required|integer|min:0|max:100',
            'margin_left' => 'required|integer|min:0|max:100',
            'margin_right' => 'required|integer|min:0|max:100',
