...
.tb-btn:hover { background: #f0f4f8; color: #1e293b; }
.dark .tb-btn:hover { background: #2d2d32; color: #f4f4f5; }
.tb-btn.is-active { background: #fef3c7; color: #92400e; box-shadow: inset 0 1px 3px rgba(0,0,0,.08); }
.dark .tb-btn.is-active { background: #422006; color: #fcd34d; }

.tb-sep { width: 1px; height: 22px; background: #e2e8f0; margin: 0 5px; flex-shrink: 0; }
...
.tb-select:focus { border-color: #f59e0b; box-shadow: 0 0 0 2px rgba(245,158,11,.12); }
.dark .tb-select { background: #232327; border-color: #2d2d32; color: #d4d4d8; }
.dark .tb-select:focus { border-color: #f59e0b; }
...
<div x-data="editTemplateState()" x-init="init()" class="h-full flex flex-col md:flex-row overflow-hidden">
        ...
        <form id="template-form" action="{{ route('letters.update', $template->id) }}" method="POST" class="flex flex-col h-full overflow-hidden" @submit="syncContent()">
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <div class="flex items-start justify-between">
                    <div>
                        <h2 class="text-sm font-bold text-slate-800 dark:text-zinc-100">Edit Template</h2>
                        <p class="text-[10px] text-slate-400 mt-0.5">Update this letter template's settings and content.</p>
                    </div>
                    <span class="text-[10px] font-mono bg-slate-100 dark:bg-zinc-800 text-slate-400 px-2 py-0.5 rounded shrink-0">#{{ $template->id }}</span>
                </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="doc-toolbar no-print">
            <select class="tb-select" style="width:108px;" onchange="execBlock(this.value); this.blur();" id="tb-block">
            ...
            <button type="button" class="tb-btn" id="tb-italic"  onclick="exec('italic')"        title="Italic"><em>I</em></button>
            <button type="button" class="tb-btn" id="tb-under"   onclick="exec('underline')"     title="Underline"><u>U</u></button>
            <button type="button" class="tb-btn" id="tb-strike"  onclick="exec('strikeThrough')" title="Strikethrough"><s>S</s></button>

            <div class="tb-sep"></div>
            <button type="button" class="tb-btn relative overflow-visible" onclick="document.getElementById('color-inp').click()" title="Text color" style="min-width:32px;">
            ...
            <button type="button" class="tb-btn relative overflow-visible" onclick="document.getElementById('hl-inp').click()" title="Highlight" style="min-width:32px;">
                ...
                <span id="hl-bar" style="position:absolute;bottom:2px;left:4px;right:4px;height:3px;border-radius:1px;background:#fde68a;"></span>
                <input type="color" id="hl-inp" value="#fde68a" style="position:absolute;opacity:0;width:0;height:0;pointer-events:none;" oninput="execHighlight(this.value)">
            </button>

            <div class="tb-sep"></div>
            <button type="button" class="tb-btn" id="tb-left"    onclick="exec('justifyLeft')"   title="Left">
            ...
            <button type="button" class="tb-btn" id="tb-center"  onclick="exec('justifyCenter')" title="Center">
            ...
            <button type="button" class="tb-btn" id="tb-right"   onclick="exec('justifyRight')"  title="Right">
            ...
            <button type="button" class="tb-btn" onclick="exec('insertOrderedList')" title="Numbered list">
                <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24">
                    <text x="2" y="9" font-size="7.5">1.</text><text x="2" y="15.5" font-size="7.5">2.</text><text x="2" y="22" font-size="7.5">3.</text>
            ...
            <div class="tb-sep"></div>
            <button type="button" class="tb-btn" onclick="exec('indent')"  title="Indent">
            ...
            <button type="button" class="tb-btn" onclick="exec('outdent')" title="Outdent">
            ...
            <button type="button" class="tb-btn" onclick="exec('insertHorizontalRule')" title="Horizontal rule">
            ...
            <div id="pages-container">
                <div class="page-sheet">
                    <div class="a4-doc-page" contenteditable="true" spellcheck="true"
                         data-placeholder="Edit your letter content here…"></div>
                </div>
            </div>
...
function updateToolbarState() {
    const pairs = [['bold','tb-bold'],['italic','tb-italic'],['underline','tb-under'],['strikeThrough','tb-strike'],['justifyLeft','tb-left'],['justifyCenter','tb-center'],['justifyRight','tb-right'],['justifyFull','tb-justify']];
    ...
    return {
        ...
        reflowPages() {
            ...
            try {
                ...
                // Forward: push overflow to next page
                for (let i = 0; i < pages.length; i++) {
                    let guard = 200;
                    while (pages[i].scrollHeight > pages[i].clientHeight + 2 && guard-- > 0) {
                        let next = pages[i + 1];
                        if (!next) { next = this.createPage(); pages.push(next); }
                        let child = pages[i].lastChild;
                        while (child && child.nodeType === 3 && !child.textContent.trim()) child = child.previousSibling;
                        if (!child) break;
                        next.insertBefore(child, next.firstChild);
                    }
                }
                // Backward: pull content back; remove empty tail pages
                const allPgs = this.allPages();
                for (let i = allPgs.length - 1; i > 0; i--) {
                    const curr = allPgs[i], prev = allPgs[i - 1];
                    let guard = 200;
                    while (curr.firstChild && guard-- > 0) {
                        const node = curr.firstChild;
                        prev.appendChild(node);
                        if (prev.scrollHeight > prev.clientHeight + 2) { curr.insertBefore(prev.lastChild, curr.firstChild); break; }
                    }
        ...
        isPageEmpty(page) {
            return !page.firstChild || page.innerHTML.trim() === '' || page.innerHTML.trim() === '<br>';
        },

        bindPageListeners(page) {
            page.addEventListener('input', () => { this.debouncedReflow(); updateToolbarState(); });
        ...
        init() {
            this.$nextTick(() => {
                ...
                // Load saved content, distributing across pages at PAGE_BREAK markers
                const saved = @json($template->content ?? '');
                const parts = saved.split(/\n?<!-- PAGE_BREAK -->\n?/);
                first.innerHTML = parts[0] || '';
                for (let i = 1; i < parts.length; i++) {
                    const pg = this.createPage();
                    pg.innerHTML = parts[i] || '';
                }

                // Reflow after a tick so clientHeight is computed
                setTimeout(() => { this.reflowPages(); first.focus(); }, 80);
