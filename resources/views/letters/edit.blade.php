...
@section('styles')
    <style>
        ...
        .dark .doc-page-content td,
        .dark .doc-page-content th {
            border-color: #cbd5e1;
        }
        ...
        .doc-page-content td.cell-selected,
        .doc-page-content th.cell-selected {
        ...
        .dark .ctx-side-btn:hover {
            background: #27272a;
            color: #f4f4f5;
        }

        .dark .ctx-side-btn.active {
        ...
        .pop-custom-btn:hover {
            background: #e2e8f0;
        }

        .dark .pop-custom-btn:hover {
            background: #3f3f46;
        }
    ...
    <div x-data="editTemplateState()" class="flex-1 flex flex-col overflow-hidden">
        ...
        <div
            ...
            <div class="flex items-center border border-slate-200 dark:border-zinc-700 rounded-lg bg-slate-50 dark:bg-zinc-950 overflow-visible relative"
                ...
                <button type="button" @mousedown.prevent="adjustSize(-1)"
                    class="px-1.5 py-1 hover:bg-slate-200 dark:hover:bg-zinc-800 text-slate-500 cursor-pointer border-r border-slate-200 dark:border-zinc-700"><i
                ...
                <input type="number" id="tb-size" value="12" min="1" max="100"
                    ...
                    @focus="saveSelection()" @change="restoreSelection(); execFontSize(+$el.value)">
                <button type="button" @click="dropdownOpen = !dropdownOpen" @mousedown.prevent
                    class="px-1 py-1 hover:bg-slate-200 dark:hover:bg-zinc-800 text-slate-400 cursor-pointer border-r border-slate-200 dark:border-zinc-700"><i
                        class="fa-solid fa-caret-down text-xs"></i></button>
                <button type="button" @mousedown.prevent="adjustSize(1)"
                    class="px-1.5 py-1 hover:bg-slate-200 dark:hover:bg-zinc-800 text-slate-500 cursor-pointer"><i
                        class="fa-solid fa-plus text-xs"></i></button>
                <div x-show="dropdownOpen" x-transition
                    class="absolute left-1/2 -translate-x-1/2 top-full mt-1 bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl shadow-xl z-50 max-h-48 overflow-y-auto w-20 py-1"
                    style="display:none">
                    @foreach ([8, 9, 10, 11, 12, 14, 16, 18, 20, 24, 28, 32, 36, 40, 48, 60, 72, 96] as $sz)
                        <button type="button"
                            @click="restoreSelection(); execFontSize({{ $sz }}); document.getElementById('tb-size').value = {{ $sz }}; dropdownOpen = false;"
                            @mousedown.prevent
            ...
            <button type="button" @mousedown.prevent="exec('bold')" id="tb-bold"
                class="tb-btn p-1.5 text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg cursor-pointer"><i
                    class="fa-solid fa-bold text-sm"></i></button>
            <button type="button" @mousedown.prevent="exec('italic')" id="tb-italic"
                class="tb-btn p-1.5 text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg cursor-pointer"><i
                    class="fa-solid fa-italic text-sm"></i></button>
            <button type="button" @mousedown.prevent="exec('underline')" id="tb-under"
                class="tb-btn p-1.5 text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg cursor-pointer"><i
                    class="fa-solid fa-underline text-sm"></i></button>
            <button type="button" @mousedown.prevent="exec('strikeThrough')" id="tb-strike"
                class="tb-btn p-1.5 text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg cursor-pointer"><i
                    class="fa-solid fa-strikethrough text-sm"></i></button>
            ...
            <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                ...
                <div x-show="open" x-transition
                    class="absolute left-0 mt-1 p-3 bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl shadow-xl z-50 w-48 space-y-2.5"
                    style="display:none">
                    <div
                        class="flex items-center justify-between border-b border-slate-100 dark:border-zinc-800/80 pb-1.5 shrink-0">
                    ...
                    <div class="grid grid-cols-6 gap-1.5">
                        @foreach (['#000000', '#374151', '#4b5563', '#6b7280', '#9ca3af', '#d1d5db', '#ef4444', '#f97316', '#f59e0b', '#10b981', '#3b82f6', '#8b5cf6', '#ec4899', '#fca5a5', '#fdba74', '#fcd34d', '#6ee7b7', '#93c5fd'] as $color)
                            <button type="button" @click="execColor('{{ $color }}'); open = false;"
                                @mousedown.prevent
                    ...
                    <div class="border-t border-slate-100 dark:border-zinc-800/80 pt-2 flex items-center justify-between">
                        ...
                        <button type="button" @click="$refs.colorInp.click(); open = false;" @mousedown.prevent
                            class="text-[11px] font-bold text-amber-500 hover:underline cursor-pointer">Choose...</button>
                    </div>
                    <input type="color" x-ref="colorInp" class="absolute opacity-0 w-0 h-0 pointer-events-none"
                        @change="execColor($el.value)">
                </div>
            </div>
            ...
            <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                ...
                <div x-show="open" x-transition
                    ...
                    <div class="grid grid-cols-6 gap-1.5">
                        @foreach (['#fef08a', '#bbf7d0', '#bfdbfe', '#fbcfe8', '#fed7aa', '#ddd6fe', '#eab308', '#22c55e', '#3b82f6', '#ec4899', '#f97316', '#a855f7', '#facc15', '#4ade80', '#60a5fa', '#f472b6', '#fb923c', '#c084fc'] as $color)
                            <button type="button" @click="execHighlight('{{ $color }}'); open = false;"
                                @mousedown.prevent
                                class="w-5 h-5 rounded-md border border-slate-200/50 dark:border-zinc-700/50 cursor-pointer hover:scale-110 transition-transform shadow-xs"
                                style="background-color: {{ $color }}"></button>
                        @endforeach
                    ...
                    <div class="border-t border-slate-100 dark:border-zinc-800/80 pt-2 flex items-center justify-between">
                        <span class="text-[10px] font-bold text-slate-400 uppercase">Custom</span>
                        <button type="button" @click="$refs.hlInp.click(); open = false;" @mousedown.prevent
                            class="text-[11px] font-bold text-amber-500 hover:underline cursor-pointer">Choose...</button>
                    </div>
                    <input type="color" x-ref="hlInp" class="absolute opacity-0 w-0 h-0 pointer-events-none"
                        @change="execHighlight($el.value)">
                </div>
            </div>

            <div class="h-5 w-px bg-slate-200 dark:bg-zinc-700 mx-0.5"></div>

            <button type="button" @mousedown.prevent="exec('justifyLeft')" id="tb-left"
                class="tb-btn p-1.5 text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg cursor-pointer"><i
                    class="fa-solid fa-align-left text-sm"></i></button>
            <button type="button" @mousedown.prevent="exec('justifyCenter')" id="tb-center"
                class="tb-btn p-1.5 text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg cursor-pointer"><i
                    class="fa-solid fa-align-center text-sm"></i></button>
            <button type="button" @mousedown.prevent="exec('justifyRight')" id="tb-right"
                class="tb-btn p-1.5 text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg cursor-pointer"><i
                    class="fa-solid fa-align-right text-sm"></i></button>
            <button type="button" @mousedown.prevent="exec('justifyFull')" id="tb-justify"
                class="tb-btn p-1.5 text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg cursor-pointer"><i
                    class="fa-solid fa-align-justify text-sm"></i></button>
            ...
            <button type="button" @mousedown.prevent="exec('insertUnorderedList')"
                class="p-1.5 text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg cursor-pointer"
                title="Bullet List"><i class="fa-solid fa-list-ul text-sm"></i></button>
            <button type="button" @mousedown.prevent="exec('insertOrderedList')"
                class="p-1.5 text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg cursor-pointer"
                title="Numbered List"><i class="fa-solid fa-list-ol text-sm"></i></button>
            ...
            <button type="button" @mousedown.prevent="exec('insertHorizontalRule')"
                class="p-1.5 text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg cursor-pointer"
                title="HR"><i class="fa-solid fa-minus text-sm"></i></button>
            ...
            <button type="button" @mousedown.prevent="exec('removeFormat')"
                class="p-1.5 text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg cursor-pointer"
                title="Clear Format"><i class="fa-solid fa-eraser text-sm"></i></button>
        ...
        <div id="table-ctx-menu">
            <div class="ctx-tb-row">
                ...
                <button type="button" class="ctx-tb-btn ctx-danger" data-tt="Delete Row"
                ...
                <button type="button" class="ctx-tb-btn" data-tt="Add Column Right"
                    onclick="tableAction('addColRight')">
                    <i class="fa-solid fa-arrow-right fa-xs"></i>
                </button>
                <button type="button" class="ctx-tb-btn ctx-danger" data-tt="Delete Column"
                    onclick="tableAction('deleteCol')">
                    <i class="fa-solid fa-trash fa-xs"></i>
                </button>
                <div class="ctx-tb-divider"></div>
                <button type="button" class="ctx-tb-btn" id="tb-btn-merge" data-tt="Merge Cells"
                    onclick="tableAction('mergeCells')">
                    <i class="fa-solid fa-object-group fa-xs"></i>
                </button>
                <button type="button" class="ctx-tb-btn" id="tb-btn-unmerge" data-tt="Unmerge Cells"
                    onclick="tableAction('unmergeCells')">
                    <i class="fa-solid fa-object-ungroup fa-xs"></i>
                </button>
            </div>
            <div class="ctx-tb-row">
                <div class="ctx-side-btns">
                    <button type="button" class="ctx-side-btn active" id="side-all" data-tt="All Borders"
                        onclick="toggleBorderSide('all')">All</button>
                    <button type="button" class="ctx-side-btn" id="side-inside" data-tt="Inside Borders"
                        onclick="toggleBorderSide('inside')">Inside</button>
                    <button type="button" class="ctx-side-btn" id="side-top" data-tt="Top Border"
                        onclick="toggleBorderSide('top')">Top</button>
                    <button type="button" class="ctx-side-btn" id="side-bottom" data-tt="Bottom Border"
                        onclick="toggleBorderSide('bottom')">Bot</button>
                    <button type="button" class="ctx-side-btn" id="side-left" data-tt="Left Border"
                        onclick="toggleBorderSide('left')">Left</button>
                    <button type="button" class="ctx-side-btn" id="side-right" data-tt="Right Border"
                        onclick="toggleBorderSide('right')">Right</button>
                </div>
                <div class="ctx-tb-divider"></div>
                <button type="button" class="ctx-color-btn" data-tt="Border Color"
                    onclick="openSwatchPopover('border', this)">
                    <i class="fa-solid fa-border-all fa-xs"></i>
                    <span class="ctx-swatch-dot" id="cell-border-swatch" style="background:#cbd5e1"></span>
                </button>
                <button type="button" class="ctx-color-btn" data-tt="Background Color"
                    onclick="openSwatchPopover('bg', this)">
        ...
        <input type="color" id="cell-border-inp"
            style="position:fixed;opacity:0;pointer-events:none;width:1px;height:1px;z-index:99999" value="#cbd5e1"
            oninput="tableAction('cellBorder',this.value)" onchange="tableAction('cellBorder',this.value)">
        <input type="color" id="cell-bg-inp"
            style="position:fixed;opacity:0;pointer-events:none;width:1px;height:1px;z-index:99999" value="#ffffff"
            oninput="tableAction('cellBg',this.value)" onchange="tableAction('cellBg',this.value)">
        ...
        <form id="template-form" action="{{ route('letters.update', $template->id) }}" method="POST"
            ...
            <aside
                class="no-print w-80 bg-white dark:bg-zinc-900 border-r border-slate-200 dark:border-zinc-800 flex flex-col overflow-hidden shrink-0 shadow-sm z-20">
                <!-- Document Settings section -->
                <div class="p-5 border-b border-slate-200 dark:border-zinc-800 shrink-0 space-y-3">
                    <h3 class="text-sm font-bold text-slate-800 dark:text-zinc-200 uppercase tracking-wider">Document
                        Settings</h3>
                    <div class="space-y-1.5">
                        <label
                            class="block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">Template
                            Title *</label>
                ...
                <div class="p-5 border-b border-slate-200 dark:border-zinc-800 shrink-0 space-y-3">
                    <div class="flex items-center justify-between">
                        <h3 class="text-sm font-bold text-slate-800 dark:text-zinc-200 uppercase tracking-wider">
                            Placeholders</h3>
                        <p class="text-xs text-amber-600 dark:text-amber-400 font-semibold"><i
                                class="fa-solid fa-keyboard mr-1"></i>Ctrl+Enter → Break</p>
                    </div>
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-5 min-h-0">
                    ...
                    <div class="space-y-2.5 pt-4 border-t border-slate-100 dark:border-zinc-800"
                        x-show="permanentGlobalVars.length > 0">
                        <div class="flex items-center justify-between">
                            <h4
                                class="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                                <i class="fa-solid fa-lock text-emerald-500"></i> Permanent Global
                            </h4>
                            <span
                                class="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20"
                                x-text="permanentGlobalVars.length"></span>
                        </div>
                        <div class="flex flex-wrap gap-1.5">
                            ...
                            <div x-show="filteredPermanentGlobalVars.length === 0 && permanentGlobalVars.length > 0"
                                ...
                                No permanent variables match search.
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <aside
                class="no-print w-80 bg-white dark:bg-zinc-900 border-l border-slate-200 dark:border-zinc-800 flex flex-col overflow-hidden shrink-0 shadow-sm z-20">
                <!-- Header -->
                <div class="p-5 border-b border-slate-200 dark:border-zinc-800 space-y-1 shrink-0">
                    <h3 class="text-sm font-bold text-slate-800 dark:text-zinc-200 uppercase tracking-wider">Configuration
                    </h3>
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-6 min-h-0">
                    ...
                    @if ($errors->any())
                        <div
                            class="p-3 bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-800 rounded-xl text-rose-700 dark:text-rose-400 text-xs space-y-1">
                    ...
                    <!-- Page Margins -->
                    <div
                        class="p-4 bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-2xl space-y-3">
                        <div class="flex items-center justify-between">
                            <label
                                class="block text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">Page
                                Margins (mm)</label>
                            <label class="flex items-center gap-1.5 cursor-pointer select-none">
                                ...
                                <input type="checkbox" name="different_first_page_margins" value="1"
                                    x-model="differentFirstPageMargins" @change="applyMarginsToAll()"
                                    class="rounded border-slate-300 dark:border-zinc-700 text-amber-500 focus:ring-amber-500 text-xs">
                                <span class="text-[10px] font-semibold text-slate-600 dark:text-zinc-400">Diff 1st
                                    Page</span>
                            </label>
                        </div>

                        <!-- First Page Margins (shown when differentFirstPageMargins is true) -->
                        <div x-show="differentFirstPageMargins"
                            class="space-y-1.5 pb-2 border-b border-slate-200/60 dark:border-zinc-800/80">
                            <span
                                class="block text-[10px] font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400">
                                1st Page Margins
                            </span>
                            <div class="grid grid-cols-2 gap-2">
                                @foreach (['top' => 'Top', 'bottom' => 'Bottom', 'left' => 'Left', 'right' => 'Right'] as $side => $label)
                                    <div>
                                        <label
                                            class="block text-[9px] font-bold text-slate-400 mb-0.5">{{ $label }}</label>
                                        <input type="number" name="first_page_margin_{{ $side }}"
                                            x-model.number="firstPageMargins.{{ $side }}" min="0"
                                            max="100"
                        ...
                        <div class="space-y-1.5">
                            <span x-show="differentFirstPageMargins"
                                class="block text-[10px] font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400">
                                Other Pages Margins
                            </span>
                            <div class="grid grid-cols-2 gap-2">
                                @foreach (['top' => 'Top', 'bottom' => 'Bottom', 'left' => 'Left', 'right' => 'Right'] as $side => $label)
                                    <div>
                                        <label
                                            class="block text-[9px] font-bold text-slate-400 mb-0.5">{{ $label }}</label>
                                        <input type="number" name="margin_{{ $side }}"
                                            x-model.number="margins.{{ $side }}" min="0" max="100"
                                            class="w-full p-1.5 border border-slate-200 dark:border-zinc-700 rounded-lg bg-white dark:bg-zinc-900 text-xs text-center text-slate-800 dark:text-zinc-200 focus:border-amber-500 outline-none"
                                            @change="applyMarginsToAll()">
                                    </div>
                                @endforeach
                    ...
                    <!-- Page Count Display -->
                    <div
                        class="p-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/50 rounded-xl text-center">
                    ...
                    <div class="space-y-3 border-t border-slate-100 dark:border-zinc-800 pt-4">
                        ...
                        <div class="space-y-2.5">
                            <template x-for="(v, idx) in variables" :key="idx">
                                <div
                                    class="relative p-3 bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl space-y-2">
                                    <button type="button" @mousedown.prevent="removeVariable(idx)"
                                        class="absolute top-2 right-2 text-slate-400 hover:text-rose-500 cursor-pointer"><i
                                            class="fa-solid fa-xmark text-xs"></i></button>
                                    <div class="grid grid-cols-2 gap-2 pr-4">
                                        <div>
                                            <label
                                                class="block text-[9px] font-bold uppercase text-slate-400 mb-0.5">Key</label>
                                            <input type="text" x-model="v.key" placeholder="var_name"
                                                class="w-full px-2 py-1 border border-slate-200 dark:border-zinc-700 rounded-lg bg-white dark:bg-zinc-900 text-xs font-mono focus:border-amber-500 outline-none">
                                        </div>
                                        <div>
                                            <label
                                                class="block text-[9px] font-bold uppercase text-slate-400 mb-0.5">Type</label>
                                    ...
                                    <div x-show="v.type === 'dropdown'">
                                        <label
                                            class="block text-[9px] font-bold uppercase text-slate-400 mb-0.5">Options</label>
                                        <input type="text" x-model="v.options" placeholder="Option 1, Option 2"
                                            class="w-full px-2 py-1 border border-slate-200 dark:border-zinc-700 rounded-lg bg-white dark:bg-zinc-900 text-xs focus:border-amber-500 outline-none">
                                    </div>

                                    <!-- Formula sub-editor (only for 'calculated' type) -->
                                    <div x-show="v.type === 'calculated'" class="space-y-2 pt-1">
                                        <div class="flex items-center justify-between">
                                            <span class="text-[9px] font-bold uppercase text-slate-400">Formula
                                                Variables</span>
                                        ...
                                        <p class="text-[9px] text-slate-400 leading-relaxed">Expressions use JS math.
                                            Reference <code class="font-mono bg-slate-100 dark:bg-zinc-800 px-0.5 rounded"
                                                x-text="v.key || 'parent_key'"></code> or any formula key (e.g., <code
                                                class="font-mono bg-slate-100 dark:bg-zinc-800 px-0.5 rounded">gross_salary
                                                * 0.6</code>).</p>
                                        <div class="space-y-1.5">
                                            <template x-for="(f, fi) in (v.formulas || [])" :key="fi">
                                                <div
                                                    class="relative grid grid-cols-2 gap-1.5 p-2 bg-white dark:bg-zinc-900 border border-indigo-100 dark:border-indigo-900/50 rounded-lg pr-6">
                                                    ...
                                                    <div>
                                                        <label
                                                            class="block text-[8px] font-bold uppercase text-indigo-400 mb-0.5">Key</label>
                                                    ...
                                                    <div>
                                                        <label
                                                            class="block text-[8px] font-bold uppercase text-indigo-400 mb-0.5">Label
                                                            <span
                                                                class="font-normal text-slate-400">(optional)</span></label>
                                                        <input type="text" x-model="f.label"
                                                            placeholder="Basic Salary"
                                                            class="w-full px-1.5 py-1 border border-slate-200 dark:border-zinc-700 rounded bg-slate-50 dark:bg-zinc-950 text-xs font-mono focus:border-indigo-400 outline-none">
                                                    </div>
                                                    <div class="col-span-2">
                                                        <label
                                                            class="block text-[8px] font-bold uppercase text-indigo-400 mb-0.5">Expression</label>
                                                        <input type="text" x-model="f.expression"
                                                            placeholder="gross_salary * 0.6"
                                                            class="w-full px-1.5 py-1 border border-slate-200 dark:border-zinc-700 rounded bg-slate-50 dark:bg-zinc-950 text-xs font-mono focus:border-indigo-400 outline-none">
                                                    </div>
                                                    <div x-show="f.key"
                            ...
                            <div x-show="variables.length === 0"
                                class="py-5 text-center text-xs text-slate-400 italic border border-dashed border-slate-200 dark:border-zinc-800 rounded-xl">
                                No custom variables yet.</div>
                        </div>
                    </div>
                ...
                <div
                    class="p-4 border-t border-slate-200 dark:border-zinc-800 space-y-2 shrink-0 bg-white dark:bg-zinc-900">
                    <button type="submit"
                        class="w-full py-2.5 px-4 bg-amber-500 hover:bg-amber-600 text-white font-bold text-sm rounded-xl shadow-md shadow-amber-500/20 transition-all cursor-pointer">
                        <i class="fa-solid fa-floppy-disk mr-1.5"></i> Update Template
                    </button>
                    <a href="{{ route('letters.generate') }}?template_id={{ $template->id }}"
                        class="flex items-center justify-center gap-1.5 w-full py-2 px-4 border border-amber-500/30 hover:border-amber-500/60 bg-amber-500/10 hover:bg-amber-500/15 text-amber-700 dark:text-amber-400 font-bold text-xs rounded-xl transition-all">
                        <i class="fa-solid fa-wand-magic-sparkles"></i> Generate Letters
                    </a>
                    <a href="{{ route('letters.index') }}"
                        class="block text-center py-1.5 text-xs text-slate-500 dark:text-zinc-400 hover:text-slate-800 dark:hover:text-zinc-200 transition-colors">Cancel</a>
                </div>
            </aside>
        </form>
            ...
            <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden flex flex-col max-h-[85vh]"
                ...
                <div class="p-5 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between shrink-0">
                    <div class="flex items-center gap-2.5">
                        <div
                            class="w-8 h-8 rounded-xl bg-amber-500/10 text-amber-500 flex items-center justify-center text-sm font-bold">
                            <i class="fa-solid fa-book-bookmark"></i>
                        </div>
                        <div>
                            <h3 class="text-sm font-bold text-slate-900 dark:text-zinc-100 uppercase tracking-wider">Global
                                Custom Variables</h3>
                            <p class="text-[11px] text-slate-400">Select and include reusable global variables into this
                                template.</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <button type="button" @click="globalModalOpen = false"
                            class="text-slate-400 hover:text-slate-600 dark:hover:text-zinc-200 p-1 cursor-pointer">
                ...
                <div
                    class="p-4 border-b border-slate-100 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-950/50 flex items-center justify-between gap-3 shrink-0">
                            ...
                            <div class="space-y-1 min-w-0">
                                <div class="flex items-center gap-2 flex-wrap">
                                    <span class="text-xs font-bold text-slate-800 dark:text-zinc-100"
                                        x-text="gv.label"></span>
                                    <code
                                        class="text-[11px] font-mono font-bold text-amber-600 dark:text-amber-400 bg-amber-500/10 px-1.5 py-0.5 rounded"
                                        x-text="'@{{ ' + gv.key + ' }}'"></code>
                                    <span
                                        class="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.2 rounded bg-slate-100 dark:bg-zinc-800 text-slate-500 dark:text-zinc-400"
                                        x-text="gv.type"></span>
                                    <template x-if="gv.is_permanent">
                                        <span
                                            class="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.2 rounded bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20 flex items-center gap-1">
                                ...
                                <div class="flex items-center gap-3 text-[11px] text-slate-400">
                                    ...
                                    <span x-show="gv.default_value" class="font-mono">Default: <strong
                                            class="text-slate-600 dark:text-zinc-300"
                                            x-text="gv.default_value"></strong></span>
                                    <span x-show="gv.options" class="truncate max-w-xs">Options: <span
                                            class="text-slate-600 dark:text-zinc-300" x-text="gv.options"></span></span>
                                </div>
                            </div>

                            <!-- Action buttons -->
                            <div class="flex items-center gap-2 shrink-0">
                                <!-- Toggle Include in Template (Only for optional global vars) -->
                                <template x-if="gv.is_permanent">
                                    <span
                                        class="px-2.5 py-1.5 rounded-lg text-xs font-bold bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20 flex items-center gap-1">
                                        <i class="fa-solid fa-lock text-[10px]"></i> Always Active
                                    </span>
                                </template>
                                <template x-if="!gv.is_permanent">
                ...
                <!-- Modal Footer -->
                <div
                    class="p-4 border-t border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-950 flex items-center justify-between text-xs text-slate-500 dark:text-zinc-400 shrink-0">
...
@section('scripts')
    <script>
        ...
        function openSwatchPopover(type, triggerEl) {
            ...
            grid.innerHTML = '';
            const borderPresets = ['#000000', '#64748b', '#cbd5e1', '#e2e8f0', '#ef4444', '#f59e0b', '#10b981', '#3b82f6',
                '#8b5cf6', '#ec4899', '#78350f', '#0f172a'
            ];
            const bgPresets = ['transparent', '#ffffff', '#f8fafc', '#f1f5f9', '#fef3c7', '#dcfce7', '#dbeafe', '#f3e8ff',
                '#fce7f3', '#fed7aa', '#fee2e2', '#e0f2fe'
            ];
        ...
        function setupTableResizeListener(container) {
            ...
            document.addEventListener('mousemove', (e) => {
                ...
                table.querySelectorAll('tr').forEach(r => {
                    if (r.cells[curIdx]) r.cells[curIdx].style.width = ((newCurW / tableW) * 100).toFixed(
                        2) + '%';
                    if (r.cells[nextIdx]) r.cells[nextIdx].style.width = ((newNextW / tableW) * 100)
                        .toFixed(2) + '%';
                });
            });
        ...
        function updateToolbarState() {
            [
                ...
                ['italic', 'tb-italic'],
                ['underline', 'tb-under'],
                ['strikeThrough', 'tb-strike'],
                ['justifyLeft', 'tb-left'],
                ['justifyCenter', 'tb-center'],
                ['justifyRight', 'tb-right'],
                ['justifyFull', 'tb-justify']
            ]
            .forEach(([cmd, id]) => {
        ...
        function editTemplateState() {
            const rawTemplateVars = @json($template->variables ?? []);
            const allGlobalVars = @json($globalVariables ?? []);
            const permanentKeySet = new Set(allGlobalVars.filter(gv => Boolean(gv.is_permanent)).map(gv => gv.key));

            // Custom variables strictly exclude permanent global variables so they are not deleted or duplicated
            const customTemplateVars = rawTemplateVars.filter(v => {
                const k = typeof v === 'object' ? v.key : v;
                return !permanentKeySet.has(k);
            });
            ...
            return {
                ...
                prebuiltVars: [{
                    ...
                    {
                        key: 'employee_citizenship_number',
                        label: 'Citizenship No'
                    },
                    {
                        key: 'employee_citizenship_issue_date',
                        label: 'Citizenship Issue Date'
                    },
                    {
                        key: 'employee_citizenship_issue_place',
                    ...
                    {
                        key: 'employee_dp_rank',
                        label: 'DP Rank'
                    },
                    {
                        key: 'employee_tips_amount',
                        label: 'Tips Amount'
                    },
                    {
                        key: 'employee_tips_status',
                        label: 'Tips Status'
                    },
                    {
                        key: 'employee_point_value',
                        label: 'Point Value'
                    },
                    {
                        key: 'employee_tips_blank',
                        label: 'Tips Blank'
                    },
                    {
                        key: 'employee_publish_tips',
                        label: 'Publish Tips'
                    },
                    {
                        key: 'employee_tips_fixed',
                        label: 'Tips Fixed'
                    },
                    {
                        key: 'employee_his_her',
                        label: 'his/her'
                    },
                    {
                        key: 'employee_he_she',
                    ...
                    {
                        key: 'employee_him_her',
                        label: 'him/her'
                    },
                    {
                        key: 'employee_his_her_cap',
                        label: 'His/Her'
                    },
                    {
                        key: 'employee_he_she_cap',
                        label: 'He/She'
                    },
                    {
                        key: 'employee_him_her_cap',
                        label: 'Him/Her'
                    }
                ],
                ...
                globalVariables: allGlobalVars,
                get permanentGlobalVars() {
                    return this.globalVariables.filter(gv => Boolean(gv.is_permanent));
                },
                get filteredPermanentGlobalVars() {
                    if (!this.varSearch) return this.permanentGlobalVars;
                    const q = this.varSearch.toLowerCase();
                    return this.permanentGlobalVars.filter(gv =>
                        (gv.key && gv.key.toLowerCase().includes(q)) ||
                        (gv.label && gv.label.toLowerCase().includes(q)) ||
                        (gv.description && gv.description.toLowerCase().includes(q))
                    );
                },
                ...
                isPermanentVar(key) {
                    return this.permanentGlobalVars.some(gv => gv.key === key);
                },
                toggleGlobalVar(gv) {
                ...
                addAllGlobalVars() {
                    this.filteredGlobalVars.forEach(gv => {
                        if (!gv.is_permanent && !this.isVariableIncluded(gv.key)) {
                            this.variables.push({
                                key: gv.key,
                                type: gv.type || 'text',
                                dummy: gv.default_value || '',
                                options: gv.options || '',
                                formulas: Array.isArray(gv.formulas) ? JSON.parse(JSON.stringify(gv
                                    .formulas)) : []
                            });
                        }
                    });
                ...
                applyLineHeight(v) {
                    if (v) applyToCurrentBlock('lineHeight', v);
                },
                applyLetterSpacing(v) {
                    if (v !== '') applyToCurrentBlock('letterSpacing', v);
                },
                    ...
                    const splitNode = (node, usableBottom, pageRect, forceFit = false) => {
                        if (node.nodeType === 3) {
                            ...
                            if (bestSplit === node.length) {
                                ...
                                const lastSpace = Math.max(text.lastIndexOf(' ', bestSplit), text.lastIndexOf('\u00a0',
                                    bestSplit));
                ...
                createPage(explicitNum) {
                    ...
                    page.innerHTML = '<span class="page-number-label no-print" contenteditable="false">Page ' + num +
                        '</span>';
                ...
                syncContent() {
                    ...
                    pages.forEach((pageHtml, idx) => {
                        if (idx === 0) {
                            ...
                            const prevPageHadManual = rawPages[idx - 1].querySelector('.page-break-marker') !==
                            null;
                    ...
                    this.$nextTick(() => {
                        ...
                        container.addEventListener('keydown', (e) => {
                            ...
                            if (sel && sel.isCollapsed && sel.rangeCount > 0) {
                                ...
                                const content = node.nodeType === Node.ELEMENT_NODE ? node.closest(
                                    '.doc-page-content') : node.parentNode?.closest('.doc-page-content');
                                if (content) {
                                    if (e.key === 'Backspace') {
                        ...
                        container.addEventListener('mousedown', (e) => {
                            ...
                            if (cell && container.contains(cell)) {
                                initTableResizeHandles();
                                if (e.shiftKey && _lastAnchorCell && _lastAnchorCell.closest('table') ===
                                    cell.closest('table')) {
                                    e.preventDefault();
                        ...
                        container.addEventListener('click', (e) => {
                            ...
                            } else if (!e.target.closest('table') && !e.target.closest('#table-ctx-menu') &&
                                !e.target.closest('#ctx-swatch-popover')) {
                                clearCellSelection();
                                hideTableMenu();
                            }
                        });
                        ...
                        document.addEventListener('mousedown', (e) => {
                            if (e.target.closest('#pages-container') || e.target.closest(
                                '#table-ctx-menu') || e.target.closest('#ctx-swatch-popover') || e.target
                                .closest('#toolbar') || e.target.closest('.color-picker-input')) {
                                return;
                            }
                            clearCellSelection();
                            hideTableMenu();
                        ...
                        if (tgp) {
                                ...
                                for (let c = 1; c <= 6; c++) {
                                    ...
                                    cell.addEventListener('mouseover', () => {
                                        tgp.querySelectorAll('.tgp-cell').forEach(el => {
                                            el.classList.toggle('tgp-hover', +el.dataset.r <= r && +
                                                el.dataset.c <= c);
                                        });
                            ...
                            tgp.addEventListener('mouseleave', () => {
                                tgp.querySelectorAll('.tgp-cell').forEach(el => el.classList.remove(
                                    'tgp-hover'));
